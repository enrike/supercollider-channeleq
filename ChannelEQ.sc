...
ChannelEQ {
    ...
    *newBare { |frdb|
	...
	bypass { // flat the EQ
		var flat_frdb = [[100,0,1], [250,0,1], [1000,0,1], [3500,0,1], [6000,0,1]];
		flat_frdb = flat_frdb.collect({ |item, i|
            [item[0].cpsmidi - 1000.cpsmidi, item[1], item[2].log10]
        }).flat;

		if (synth.notNil and: { synth.isPlaying }) {
			synth.setn(\eq_controls, flat_frdb);
		}
	}
...
ChannelEQGUI {
	...
	var fsc, fs_toogle, inoutMenus, truebypassButton;
    ...
    *new { |channelEQ|
    ...
    init { |argChannelEQ|
		...
		.extrude_( true ).border_(1).font_( font )
		.states_( [
			[ "F", Color.black, Color.red(0.75).alpha_(0.25) ],
			[ "F", Color.black, Color.red(0.25).alpha_(0.25) ]
		] )
		.action_({ |bt| fsc.active_(bt.value.asBoolean.not) })
		.resize_(7);
		...
		*/
		...
		truebypassButton = RoundButton.new(window, 55@17)
		.extrude_(true).border_(1).font_(font)
		.states_( [
			[ "bypass", Color.black, Color.red(0.75).alpha_(0.25) ],
			[ "bypass", Color.black, Color.red(0.25).alpha_(0.25) ]
		] )
                .value_(1)
